...
async function main() {
	...
	const categories = await Promise.all([
		...
		prisma.ticketCategory.upsert({
			...
			create: {
				name: "Lainnya",
				description:
					"Keluhan atau permintaan yang tidak termasuk kategori di atas",
			},
		}),
	]);

	const [catJaringan, catHardware, catSoftware, catAkun, _catLainnya] =
		categories;
	...
	const ticket1 = await prisma.ticket.create({
		data: {
			...
			description:
				"Sejak pagi tadi laptop saya tidak bisa terhubung ke jaringan WiFi kantor. Sudah dicoba restart tapi tetap tidak bisa. Perangkat lain sepertinya bisa terhubung normal.",
	...
	const ticket2 = await prisma.ticket.create({
		data: {
			...
			description:
				'Printer di ruang administrasi menampilkan error "Paper Jam" padahal tidak ada kertas yang tersangkut. Sudah dibersihkan tapi tetap muncul pesan yang sama.',
	...
	const ticket4 = await prisma.ticket.create({
		data: {
			...
			description:
				"Setiap kali membuka menu laporan bulanan di aplikasi ERP, aplikasi langsung tertutup sendiri. Sudah terjadi sejak update versi terbaru kemarin.",
		...
		data: [
			{
				...
				fileUrl:
					"https://storage.example.com/attachments/screenshot_error_wifi.png",
				fileSize: 204800,
				mimeType: "image/png",
			...
			},
			{
				...
				fileUrl:
					"https://storage.example.com/attachments/foto_printer_error.jpg",
				fileSize: 512000,
				mimeType: "image/jpeg",
				source: AttachmentSource.CAMERA,
			},
			...
			},
			{
				...
				fileUrl:
					"https://storage.example.com/attachments/screenshot_erp_error.png",
				fileSize: 307200,
				mimeType: "image/png",
				source: AttachmentSource.CAMERA,
			},
	...
	console.log(
		`   Tickets       : 5  (1 open, 1 in_progress, 1 pending, 1 resolved, 1 closed)`,
	);
