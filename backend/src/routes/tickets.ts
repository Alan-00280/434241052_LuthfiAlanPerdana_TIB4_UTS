...
import { TicketStatus, TicketPriority } from "../generated/prisma/enums.js";
...
tickets.get("/:id", withPrisma, async (c) => {
  ...
  const ticket = await prisma.ticket.findUnique({
    where: { id },
    include: {
      creator:  { select: { id: true, fullName: true, avatarUrl: true, email: true } },
...
tickets.post("/", withPrisma, async (c) => {
  ...
  const { title, description, priority, categoryId, creatorId, attachments } = body;
...
tickets.patch("/:id/status", withPrisma, async (c) => {
  ...
  await prisma.notification.create({
    data: {
      ...
      ticketId: id,
      type:     status === "RESOLVED" ? "TICKET_RESOLVED"
