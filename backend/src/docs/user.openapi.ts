...
export const getUsersRoute = createRoute({
	...
	description:
		"Mengambil daftar semua pengguna di dalam sistem (khusus Admin).",
	security: [
		{
			BearerAuth: [],
		},
			...
			content: {
				"application/json": {
					schema: z.object({
						users: z
							.array(z.any())
							.openapi({ description: "Daftar semua pengguna" }),
					}),
				},
			},
...
export const getAssigneesRoute = createRoute({
	...
	description:
		"Mengambil semua pengguna aktif yang memiliki role ADMIN atau HELPDESK untuk ditugaskan ke tiket.",
	security: [
		{
			BearerAuth: [],
		},
			...
			content: {
				"application/json": {
					schema: z.object({
						assignees: z
							.array(z.any())
							.openapi({ description: "Daftar user assignee yang tersedia" }),
					}),
				},
			},
	...
	responses: {
		200: {
			content: {
				"application/json": {
					schema: z.object({
						techsupports: z.array(z.any()).openapi({
							description: "Daftar user tech support yang tersedia",
						}),
					}),
				},
			},
	...
	request: {
		...
		query: z.object({
			supaId: z
				.string()
				.optional()
				.openapi({ description: 'Gunakan ID Supabase UID jika diisi "true"' }),
		}),
	},
	responses: {
		200: {
...
export const updateUserRoute = createRoute({
	...
	description:
		"Memperbarui informasi profil pengguna seperti nama lengkap, nomor telepon, dan URL avatar.",
	security: [
		{
			BearerAuth: [],
		},
				...
				"application/json": {
					schema: z.object({
						fullName: z
							.string()
							.optional()
							.openapi({ description: "Nama lengkap baru" }),
						phone: z
							.string()
							.optional()
							.openapi({ description: "Nomor telepon baru" }),
						avatarUrl: z
							.string()
							.optional()
							.openapi({ description: "URL avatar baru" }),
					}),
				},
			},
		},
			...
			content: {
				"application/json": {
					schema: z.object({
						user: z
							.any()
							.openapi({ description: "Data profil user terupdate" }),
					}),
				},
			},
