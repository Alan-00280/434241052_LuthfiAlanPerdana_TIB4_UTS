...
import z from "zod";
import { TicketCategorySchema } from "../../prisma/zod/index.js";
			...
			content: {
				"application/json": {
					schema: z.object({
						attachments: z
							.array(z.any())
							.openapi({ description: "Daftar lampiran tiket" }),
					}),
				},
			},
			...
			content: {
				"multipart/form-data": {
					schema: z.object({
						changedById: z
							.string()
							.openapi({ description: "ID User pengunggah" }),
						attachments: z
							.union([z.any(), z.array(z.any())])
							.openapi({ description: "File yang diupload" }),
					}),
				},
			},
		},
			...
			content: {
				"application/json": {
					schema: z.object({
						changedById: z
							.string()
							.openapi({ description: "ID User yang menghapus" }),
					}),
				},
			},
		},
...
export const getNotificationsRoute = createRoute({
	...
	request: {
		query: z.object({
			userId: z
				.string()
				.openapi({ description: "ID User penerima notifikasi" }),
			supaId: z
				.string()
				.optional()
				.openapi({ description: "Gunakan ID Supabase UID jika diisi" }),
			unread: z.string().optional().openapi({
				description: 'Set "true" untuk memfilter hanya notifikasi belum dibaca',
			}),
		}),
	},
	responses: {
				...
				"application/json": {
					schema: z.object({
						...
						description: z
							.string()
							.optional()
							.openapi({ description: "Deskripsi kategori baru" }),
					}),
				},
			},
		},
				...
				"application/json": {
					schema: z.object({
						name: z
							.string()
							.optional()
							.openapi({ description: "Nama kategori baru" }),
						description: z
							.string()
							.optional()
							.openapi({ description: "Deskripsi kategori baru" }),
