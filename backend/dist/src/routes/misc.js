...
import { getAttachmentsRoute, createAttachmentRoute, deleteAttachmentRoute, getNotificationsRoute, readNotificationRoute, readAllNotificationsRoute, getCategoriesRoute, createCategoryRoute, updateCategoryRoute, deleteCategoryRoute, getHistoriesRoute, } from "../docs/misc.openapi.js";
// ─────────────────────────────────────────
// ATTACHMENTS
// ─────────────────────────────────────────
...
attachments.openapi(createAttachmentRoute, async (c) => {
    ...
    const changedById = formData.get("changedById");
    if (!changedById)
        return c.json({ error: "Changed by Id Wajib di isi" }, 401);
    if (!ticketId) {
...
// NOTIFICATIONS
// ─────────────────────────────────────────
export const notifications = new OpenAPIHono();
...
categories.openapi(createCategoryRoute, async (c) => {
    ...
    if (!name)
        return c.json({ error: "name wajib diisi" }, 400);
...
// ─────────────────────────────────────────
export const histories = new OpenAPIHono();
