...
import { getTicketsRoute, getStatsRoute, getTicketDetailRoute, createTicketRoute, updateTicketRoute, updateTicketStatusRoute, assignTicketRoute, deleteTicketRoute, } from "../docs/ticket.openapi.js";
...
tickets.openapi(createTicketRoute, async (c) => {
    ...
    if (contentType.includes("multipart/form-data") || contentType.includes("application/x-www-form-urlencoded")) {
        ...
        files = rawFiles.filter((file) => file &&
            typeof file === "object" &&
            "name" in file &&
            "size" in file &&
    ...
    if (!title || !description || !creatorId) {
        return c.json({ error: "title, description, dan creatorId wajib diisi" }, 400);
    }
...
tickets.openapi(assignTicketRoute, async (c) => {
    ...
    if (!assigneeId || !changedById || !techSupportId) {
        return c.json({ error: "assigneeId dan changedById dan techSupportId wajib diisi" }, 400);
    }
    ...
    if (!assignee)
        return c.json({ error: "Assignee tidak ditemukan" }, 404);
    ...
    const ticket = await prisma.ticket.update({
        where: { id },
        data: { assigneeId, techSupportId: techSupport.id, status: TicketStatus.IN_PROGRESS },
        include: {
